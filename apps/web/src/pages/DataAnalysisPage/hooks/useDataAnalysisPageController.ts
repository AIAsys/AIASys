...
import { useDataAnalysisOverlayState } from "./useDataAnalysisOverlayState";
import { useDataAnalysisPageEffects } from "./useDataAnalysisPageEffects";
...
interface UseDataAnalysisPageControllerOptions {
  userId: string;
...
export function useDataAnalysisPageController({
  userId,
  initialSessionId,
}: UseDataAnalysisPageControllerOptions) {
  const apiBaseUrl = API_BASE_URL || "";
  const overlayState = useDataAnalysisOverlayState();
  ...
  const {
    currentRequest: askUserRequest,
    isLoading: isAskUserLoading,
    requestError: askUserRequestError,
    showAskUser,
    resolveRequestById,
    dismissCurrentRequest: dismissAskUserRequest,
  ...
  // 同步 ref，避免 useCodeExecutor 初始化时捕获旧回调
  useEffect(() => {
    handleAskUserRequestRef.current = handleAskUserRequest;
  }, [handleAskUserRequest]);

  const handleAskUserResolve = useCallback(
    async (approved: boolean, value?: AskUserValue) => {
      if (!askUserRequest) {
        return false;
      }
      return resolveRequestById(askUserRequest.request_id, approved, value);
    },
    [askUserRequest, resolveRequestById],
  );
  ...
  return {
    ...
    askUser: {
      ...
      isLoading: isAskUserLoading,
      error: askUserRequestError,
      handleResolve: handleAskUserResolve,
      dismiss: dismissAskUserRequest,
    },
    toolPreview: {
