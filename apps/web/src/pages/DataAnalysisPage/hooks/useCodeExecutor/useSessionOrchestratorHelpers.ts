...
function getCurrentAnalysisWorkspaceId(): string | null {
  if (globalThis.location.pathname.replace(/\/+$/, "") !== "/analysis") {
    return null;
  }
...
export function navigateToAnalysisSession(
  ...
  const url = new URL("/analysis", globalThis.location.origin);
  const workspaceId =
    options?.workspaceId === undefined
      ? getCurrentAnalysisWorkspaceId()
      : options.workspaceId;
