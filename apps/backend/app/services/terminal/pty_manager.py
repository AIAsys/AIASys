...
class PtyManager:
    ...
    async def spawn(
        ...
        if not PTY_SUPPORTED:
            raise PtyUnsupportedError("当前平台不支持内置 PTY 终端，请使用外部终端。")
        ...
        logger.info(
            "PTY 启动: terminal_id=%s pid=%d cols=%d rows=%d cwd=%s",
            terminal_id,
            pid,
            cols,
            rows,
    ...
    @staticmethod
    async def _read_loop(session: PtySession) -> None:
        """后台读取循环：从 PTY master fd 读取输出"""
        ...
        logger.debug(
            "PTY 读取循环结束: terminal_id=%s exit_code=%d", session.terminal_id, exit_code
        )
