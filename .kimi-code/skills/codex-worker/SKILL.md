...
一旦真的启动并行 worker，主控必须同步更新当前 active task session。
...
如果当前任务还没有这张表，先补台账，再继续派工或汇报。

更完整的并行 worktree 工作流，读取：

- `../git-workflow/references/parallel-worktree-development.md`
