...
### 规则 14：Agent 记忆不是“配置模板继承”，必须分层为“用户画像”“环境事实”“历史召回”三层

AIASys 当前的 `PREFERENCES.md` + `.preference.md` 只解决了“配置模板复制”，这还不算完整的 Agent 记忆系统。设计或评估记忆机制时，必须确认是否覆盖了下面三个问题：
