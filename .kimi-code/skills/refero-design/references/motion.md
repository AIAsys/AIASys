...
- 错误： **300–600ms on hover/buttons** — feels sluggish
- 错误： **Linear easing** — robotic, unnatural
- 错误： **Everything animates at once** — no attention hierarchy
- 错误： **Infinite loops in product screens** — distracting
- 错误： **Inconsistent timings** — 200ms here, 400ms there, chaos
- 错误： **Large movements without reduced-motion** — accessibility fail
- 错误： **Bounce/spring that doesn't settle** — "jello" effect
- 错误： **Animation for animation's sake** — if you can't justify it, remove it

### Critical: Never Use `transition: all`

```css
/* 错误： NEVER — unpredictable, causes layout thrashing */
.button {
  transition: all 200ms ease;
}

/* Good: ALWAYS — explicit properties only */
.button {
  transition: background-color 120ms var(--ease-out),
              transform 90ms var(--ease-out);
}
