...
`refero-design` is the canonical design-research skill in this repo.

If the ask is only:

- “给我一些设计系统参考”
- “找几个成熟组件库例子”
- “有哪些现成 design system 可参考”

也优先从这里进入，而不是再维护一份独立的方法论。

轻量场景下，直接读取：

- `references/design-systems-reference.md`

需要真实产品 screen / flow 研究时，再继续走 Refero MCP 工作流。
...
- 正确： "pricing toggle", "testimonial carousel", "feature comparison table"
- 正确： "Stripe", "Linear", "Notion" (company names)
- 正确： "dark mode", "minimalist", "gradient" (visual styles)
- 正确： "ios", "web", "android" (platforms)
- 错误： "fintech onboarding", "enterprise SaaS" (industry is in metadata, not descriptions)
- 错误： "user-friendly pricing" (subjective, not searchable)
...
**提示:** Call for 2-3 screens at a time, not all at once. If something fails—try smaller batches, don't stop.

错误： Reading search descriptions only = surface research
正确： Calling `get_screen` for best finds = real research
...
**Common mistakes:**
- 错误： All queries about same topic ("paywall", "subscription paywall", "iOS paywall")—vary by companies, elements, styles, colors, platforms
- 错误： Reading only search descriptions without calling `get_screen`
- 错误： Stopping at first 10-20 results when deeper exploration would help
- 错误： Hitting a problem and giving up—try smaller batches, different approach, keep going
...
- 错误： "Linear — clean design" (not actionable)
- 正确： "Linear — 13px/20px body text, -0.01em tracking, 48px section gaps, #5E6AD2 accent at 8% opacity for hover states"
...
**Common mistakes:**
- 错误： ALL CAPS without tracking (looks cramped)
- 错误： Small text without positive tracking (hard to read)
...
> **(禁止) NO INDIGO/VIOLET** — Unless the user explicitly requests purple/indigo, do not use it. Every LLM defaults to indigo (#6366f1) because it's "safe." This makes it the biggest tell of AI-generated design. Choose brand-appropriate colors from your research instead.

**Generic AI output:**
- 错误： Default system fonts without intention
- 错误： Safe blue gradients everywhere
- 错误： Perfect symmetry (no visual tension)
- 错误： Blob/wave backgrounds (meaningless decoration)
- 错误： Stock illustrations that could be anywhere

**Professional design:**
- 正确： Brand-appropriate color chosen from research
- 正确： Intentional font pairing that matches tone
- 正确： Visual tension and asymmetry
- 正确： Purposeful whitespace
- 正确： Custom or curated imagery
- 正确： At least one "clever" element from Steal List implemented
- 正确： Social proof present (or justified absence)
...
**Anti-patterns:** 错误： 300ms+ on hover 错误： Linear easing 错误： Everything animates at once
