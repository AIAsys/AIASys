...
### Multi-Artboard Merge Strategy

When a design set accumulates many screens across multiple `.pen` files, use the **merge script** to aggregate them into a single file for easy browsing:

```
design/frontend/<design-set>/
├── <design-set>-v1.pen        # Aggregated — ALL artboards in one view
├── artboards/                  # Staging — individual artboards
│   ├── workspace-home.pen
│   ├── mcp-market-page.pen
│   └── ...
├── merge_artboards.py          # python3 merge_artboards.py --write
└── README.md
```

Workflow: design in `artboards/` → merge into aggregated file → review all screens in one Pencil view.

The merge script ([scripts/merge_artboards.py](scripts/merge_artboards.py)):
- Run: `python3 .agents/skills/pencil-design/scripts/merge_artboards.py design/frontend/<design-set> --write`
- Dry-run: omit `--write`
- Auto-detects aggregated `.pen` file (prefers `*-v*.pen`)
- Skips duplicates (same artboard `id`), appends new, grid positioning (4 cols)
- Auto-fixes smart/curly quotes (U+201C/U+201D)

Artboard ID convention: `artboard-<feature>-<view>` (e.g. `artboard-mcp-market-page`).
