...
`~/.kimi/config.toml` 的完整配置项说明。支持 TOML 和 JSON 两种格式。
...
| 方式 | 说明 |
|------|------|
| 默认位置 | `~/.kimi/config.toml` |
| 指定文件 | `kimi --config-file /path/to/config.toml` |
| 内联配置 | `kimi --config '{"default_model": "..."}'` |
...
| `default_thinking` | bool | `false` | 默认开启思考模式 |
| `default_yolo` | bool | `false` | 默认开启 YOLO（自动审批）模式 |
...
控制后台任务（`Shell(run_in_background=true)` 和 `Agent(run_in_background=true)`）的运行时行为。
...
**保护敏感文件**（`.kimi/hooks/protect-env.sh`）：
...
**自动格式化代码**（`.kimi/hooks/auto-format.sh`）：
...
**检查未完成任务**（`.kimi/hooks/check-complete.sh`）：
...
两者可结合使用：通过 hook 阻止某些操作，通过插件提供替代方案。
...
| `explore` | 快速只读探索 | Shell、ReadFile、Glob、Grep、SearchWeb、FetchURL（无写入工具） |
| `plan` | 实现规划 | ReadFile、Glob、Grep、SearchWeb、FetchURL（无 Shell、无写入） |
