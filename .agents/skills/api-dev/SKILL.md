...
**不覆盖：**
- Agent 流式输出和 SSE 路由（参考 `sse-real-time-communication`）
- Agent Tool 的开发（参考 `agent-dev`）
- 数据库模型设计细节（参考 `database-model`）
- API 的测试编写（参考 `testing-strategy`）
- 系统设计层的作用域分层和配置模型（参考 `aiasys-system-design`）

### 关联文档

- `database-model`：数据库模型定义、关系设计、索引策略
- `testing-strategy`：API 端点的 pytest 测试模式
- `aiasys-system-design`：后端能力模型、配置模型、作用域分层
- `sse-real-time-communication`：Agent 流式输出的 SSE 路由
- `agent-dev`：Agent Tool 的开发规范
