...
1. **每个迁移只做一件事**：一个 migration 文件对应一次明确的 schema 变更，不要在一个 migration 里混合多个无关操作
2. **迁移必须可回滚**：每个 `upgrade()` 必须有对应的 `downgrade()`，且回滚后 schema 状态与迁移前完全一致
3. **生产环境数据迁移要格外小心**：涉及数据变更（非纯 schema 变更）的 migration，先在 staging 环境验证，确认数据转换正确后再上生产
