...
```css
/* Bad: Shows focus ring on mouse click — annoying */
.button:focus {
  outline: 2px solid var(--primary);
}

/* Good: Shows focus ring only on keyboard navigation */
...
```css
/* Bad: NEVER — breaks keyboard navigation */
.button:focus {
  outline: none;
}

/* Good: Replace default with custom visible focus */
.button:focus-visible {
  outline: none;
...
```html
<!-- Good: Correct types trigger right keyboard on mobile -->
<input type="email" inputmode="email" autocomplete="email">
<input type="tel" inputmode="tel" autocomplete="tel">
<input type="url" inputmode="url">
<input type="number" inputmode="numeric">

<!-- Good: Meaningful names help password managers -->
...
```jsx
/* Bad: NEVER — accessibility violation, user hostile */
<input onPaste={(e) => e.preventDefault()} />

/* Good: Let users paste */
<input />
```
...
```html
<!-- Good: Clickable label (explicit) -->
<label for="email">Email</label>
<input id="email" type="email">

<!-- Good: Clickable label (implicit) -->
<label>
  Email
  <input type="email">
</label>

<!-- Good: Checkbox: label + control share single hit target -->
...
```html
<!-- Good: Placeholders end with … and show format -->
...
```jsx
/* Good: Button enabled until request starts, then shows spinner */
<button 
  type="submit" 
...
```jsx
/* Good: Errors inline, focus first error on submit */
...
/* Good: No re-renders during typing */
...
```jsx
/* Good: Filters, tabs, pagination in URL */
...
```jsx
/* Bad: onClick navigation — breaks Cmd+click, middle-click */
<div onClick={() => navigate('/page')}>Go</div>

/* Good: Proper link — all browser features work */
