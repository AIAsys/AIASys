...
## Multi-Artboard File Strategy

### Problem

When designing many screens, creating one `.pen` file per artboard leads to scattered files that are hard to browse and compare. Opening each file separately in Pencil is slow and breaks the visual flow.

### Strategy: Single Aggregated File + Artboard Staging

Use **one aggregated `.pen` file** as the design deliverable, with an `artboards/` staging directory for individual work:

```
design/frontend/<design-set>/
├── <design-set>-v1.pen        # Aggregated file — ALL artboards in one view
├── artboards/                  # Staging — individual artboards (one .pen per file)
│   ├── workspace-home.pen
│   ├── mcp-market-page.pen
│   └── ...
├── merge_artboards.py          # Merge script
└── README.md
```

**Workflow:**

1. **Design** new artboards in `artboards/` as individual `.pen` files (one frame per file)
2. **Merge** into the aggregated file with `python3 merge_artboards.py --write`
3. **Review** the aggregated file in Pencil — all screens visible side-by-side in a grid

**Merge script** ([scripts/merge_artboards.py](scripts/merge_artboards.py)):

- Run from project root: `python3 .agents/skills/pencil-design/scripts/merge_artboards.py design/frontend/<design-set> --write`
- Dry-run (preview only): omit `--write`
- Auto-detects the aggregated `.pen` file (prefers `*-v*.pen` naming)
- Extracts the single top-level frame from each `artboards/*.pen`
- Skips duplicates (same artboard `id` already in aggregated file)
- Appends new artboards and repositions all frames in a grid layout (4 columns)
- Auto-fixes smart/curly quote issues (U+201C/U+201D) in source files

### When to Create a New Artboard File

- Each artboard should represent **one distinct screen, page, or modal state**
- Use a clear, descriptive file name: `<feature>-<view>-<state>.pen`
- Set a unique `id` on the top-level frame to avoid merge conflicts
- Share the same `variables` and `themes` across all artboard files for consistency

### Artboard ID Convention

Use `artboard-<feature>-<view>` pattern for frame IDs:

| Pattern | Example |
|---------|---------|
| `artboard-<page>-page` | `artboard-mcp-market-page` |
| `artboard-<feature>-<state>` | `artboard-analysis-right-open` |
| `artboard-<feature>-modal` | `artboard-branch-settings` |
