...
- 正例：`feat: 上传全部代码至 li-xiu-qi 分支` 或 `chore: 将入口文件移入 gitignore`
- 禁止写"修复若干问题""更新代码""各种改动"这类空泛描述
- 如果一次 commit 包含多种改动，优先拆分提交；确实不能拆分时，message 要覆盖所有主要改动
- **禁止泄露被 gitignore 的内容**：commit message 中不得出现 `.agents/skills/` 下的具体 skill 名称、`.agents/` 目录路径、`design-draft/` 路径、`AGENTS.md` 等被 gitignore 的文件名或目录名。用泛化描述替代，如用"桌面打包规范"代替"desktop-packaging skill"，用"设计文档"代替"design-draft/design/..."
- **禁止泄露内部协作机制**：commit message 中不得出现"skill 同步检查""skill 关联引用""AGENTS.md 约束"等指向内部协作流程的表述
...
代码变更如果涉及用户可见行为、skill 职责范围、或 AGENTS.md 约束规则，必须同步检查文档和 skill 的一致性。

**检查清单：**

| 检查项 | 触发条件 | 动作 |
|--------|----------|------|
| 用户文档 | 修改了用户可见的行为、UI 流程、配置项、API | 检查 `docs/guides/` 下对应文档是否需要更新 |
| 设计文档 | 修改了架构、对象模型、交互模式、运行态设计 | 检查 `design-draft/design/` 下对应文档是否需要更新 |
| AGENTS.md | 修改了项目语义、协作规则、多终端约束、打包规则等 | 同步更新 AGENTS.md 对应章节 |
| Skill 引用 | 新增/修改/废弃了 skill | 按 `todo-orchestration` 的"文档同步提醒"执行关联引用检查 |
| 映射索引 | 新增/删除了前端组件或后端路由 | 同步更新 `docs/guides/DOC_CODE_MAP.md` |
| Changelog | 功能变更或 bug 修复 | 确认 `docs/changelog/` 或 `design-draft/change-log/drafts/` 已记录 |

**快速判断方法：**

1. 运行 `git diff --name-only` 查看本次改动的文件列表
2. 如果有 `apps/web/src/components/`、`apps/backend/app/api/` 等用户-facing 代码，检查对应功能文档
3. 如果有涉及架构变更、对象模型调整、交互流程改动，检查 `design-draft/design/` 下对应设计文档
4. 如果有 `.agents/skills/` 下的改动，执行 skill 关联引用检查
5. 如果有 `AGENTS.md` 改动，检查受影响的 skill 是否需要同步（注意：AGENTS.md 被 gitignore，改动不进入版本控制，但 skill 引用检查仍然要做）
6. 如果以上都未触发，在 commit message 中注明"无文档变更"

**关于 design 文档的重要性：**

`design-draft/design/` 是 AIASys 的设计事实源，记录架构决策、对象模型、接口契约和实现约束。虽然 `design-draft/` 被 gitignore，但它是开发记忆的核心组成部分，必须与代码保持同步。改了代码但 design 文档未更新，会导致后续开发者（包括 AI）读到过时的设计假设，产生不可预期的行为。

**禁止行为：**
- 改了用户-facing 功能但文档未同步就 push
- 改了架构或对象模型但 design 文档未同步就 push
- 改了 skill 的 description 或职责范围但不检查其他 skill 的引用
- 改了 AGENTS.md 约束但不检查 skill 中是否有冲突描述

**commit message 泄露防护（同步检查的延伸）：**

文档同步检查完成后，构造 commit message 时还需过一遍泄露检查：
- commit message 中是否出现了 `.agents/skills/` 下的具体 skill 名称？如有，替换为泛化描述
- commit message 中是否出现了 `design-draft/`、`AGENTS.md`、`.agents/` 等被 gitignore 的路径？如有，删除或替换
- commit message 中是否出现了"skill 同步""skill 引用检查""AGENTS.md 约束"等内部协作机制术语？如有，替换为面向改动的描述
- 泛化替换示例：`desktop-packaging skill` → `桌面打包规范`；`design-draft/design/terminal.md` → `终端设计文档`；`skill 同步检查` → `文档同步检查`
...
**被 `.gitignore` 排除的文件和目录不应以任何方式泄露到远程仓库。**

这包括两个层面：
1. **对外文档不得引用**：被 gitignore 的内容不应在 README、docs、CONTRIBUTING 等对外可见的文档中出现
2. **commit message 不得泄露**：commit message、PR 标题/描述、issue 内容、changelog 中同样不得出现被 gitignore 的文件名、目录名或内部机制名称
...
- commit message、PR 标题/描述中不得出现上述被 gitignore 的文件名、目录名或内部机制名称
...
3. 文档与 skill 同步检查通过（见上文"提交前文档与 skill 同步检查"）
4. 没有未推送的本地 commit
5. changelog 已更新（如果是功能变更）
...
# 3. 确认文档与 skill 同步检查通过
# - 检查 docs/guides/ 下对应文档是否需要更新
# - 检查 .agents/skills/ 下关联引用是否失效
# - 检查 AGENTS.md 是否需要同步
...
## 文档同步
- [x] 用户文档已更新（如涉及用户可见变更）
- [x] Skill 关联引用已检查（如涉及 skill 变更）
- [x] AGENTS.md 已同步（如涉及约束规则变更）
...
- [ ] commit message / PR 标题中有没有泄露被 gitignore 的内容（`.agents/`、`design-draft/`、`AGENTS.md` 等文件名、目录名、skill 具体名称）
